...
export function createApp(ctx: AppContext): express.Express {
  ...
  app.post('/api/confirmations/:id/approve', async (req, res) => { try { const expected = typeof req.body?.actionHash === 'string' ? req.body.actionHash : ''; const confirmation = approveConfirmation(ctx.db, res.locals.user!.id, req.params.id, expected); const result = await ctx.runner.run(res.locals.user!.id, res.locals.requestId!, confirmation.toolName, confirmation.args, confirmation.id, confirmation.operationId); markUsed(ctx.db, confirmation.id); await audit(ctx, 'confirmation.approved', res.locals.user!.id, { confirmationId: confirmation.id, result: result.ok }); return res.json({ confirmation, result }); } catch (error) { return res.status(400).json({ error: error instanceof Error ? error.message : 'Confirmation failed' }); } });
