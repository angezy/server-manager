...
export function classifyIntent(prompt: string): IntentHint {
  ...
  if (/\b(?:deploy|deployment|release|publish|provision)\b/i.test(value) && /\b(?:app|application|node|pm2|nginx|certbot|domain|site|website|server)\b/i.test(value)) return 'deployment';
...
function systemInstructions(intent: IntentHint): string {
  return `You are Sentinel, a cautious infrastructure assistant. Initial intent hint: ${intent}. Use the typed tools to gather evidence, then reason over tool results. Never emit or execute shell commands. Never invent tool results. Read-only tools may run without confirmation; every modifying tool requires confirmation and the exact arguments must be reviewed. Respect path, service, container, deployment, and risk allowlists. For deploy_node_app, never guess the local port or Certbot email; ask the user when either is missing. Do not read or reveal secrets, .env files, API keys, passwords, or private keys. Do not use SITE_DOMAIN unless the user explicitly asks about a website or domain. Return a concise final answer with evidence, uncertainty, and whether any action was performed.`;
}
...
function confirmationTarget(args: Record<string, unknown>): string { if (typeof args.processName === 'string' && typeof args.domain === 'string') return `app=${args.path}; pm2=${args.processName}; domain=${args.domain}; port=${args.port}; certbotEmail=${args.certbotEmail}`; const target = args.path ?? args.service ?? args.unit ?? args.name; if (typeof args.content === 'string') return `${String(target ?? 'exact target')} (${args.content.length} bytes; content is held server-side and is not shown here)`; return typeof target === 'string' ? target : safeJson(args); }
