...
const canonicalDefinitions: Record<CanonicalName, ToolDefinition> = {
  ...
  deploy_node_app: modify('deploy_node_app', 'Safely start one allowlisted Node app with PM2, create and enable an exact Nginx site, validate and reload Nginx, then request a Certbot certificate.', ToolArgs.deployNodeApp, object({ path: string('Exact allowlisted application directory containing package.json.'), processName: string('Exact allowlisted PM2 process name.'), domain: string('Exact allowlisted domain to configure.'), port: { type: 'integer', description: 'Local application port.', minimum: 1024, maximum: 65535 }, certbotEmail: string('Email for the Let\'s Encrypt registration.') }, ['path', 'processName', 'domain', 'port', 'certbotEmail']), 'CRITICAL')
};
